...
public class PlayerStats : MonoBehaviour
{
    ...
    public void UpdateUI()
    {
        if (hpSlider != null)
        {
            // HP Bar reflects the total current battle HP (Base + Temp Buff)
    ...
    public void GainEXP(int expAmount)
    {
        ...
        while (currentEXP >= expToNextLevel) // Handle multiple level-ups
        {
            currentEXP -= expToNextLevel;
            CurrentLevel++;
            expToNextLevel = (int)(expToNextLevel * 1.5f);
            LevelUpStats(); // Applies permanent stats change
        }

        if (CurrentLevel > oldLevel) // Check if the level has actually increased
        {
            SavePermanentStats(); // <-- Save the new level and base stats
            OnLevelUp?.Invoke();
        }
        // Even without leveling, save the accumulated EXP
        SavePermanentStats();
    }
    ...
    private void LevelUpStats()
    {
        ...
        // Note: We are modifying baseAttack and baseMaxHP directly, making the change permanent.
        if (CurrentLevel <= 10)
        {
            attackIncrease = 2;
            hpIncrease = 3;
        ...
        // Apply PERMANENT stat changes directly to the base stats
        baseAttack += attackIncrease;
        baseMaxHP += hpIncrease;
        currentHP += hpIncrease; // Heal to full effective HP on level up
