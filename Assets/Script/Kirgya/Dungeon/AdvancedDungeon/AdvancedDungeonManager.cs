...
public class AdvancedDungeonManager : MonoBehaviour
{
    ...
    public AdvancedDungeonQuizManager dungeonQuiz;
    ...
    // --- EDITED: NextLevel() Logic for 7-Rest-6-Rest-7-Rest-Boss Flow ---
    ...
    // --- EDITED: GetEncounterTitle() Logic for new level structure ---
    public string GetEncounterTitle(EnemyStats enemy, BossStats boss)
    {
        // Rest Phases
        if (CurrentLevel == 8 || CurrentLevel == 15 || CurrentLevel == 23)
        {
            return "Rest Phase";
        }
        // Boss Phase
        ...
        else
        {
            ...
            // Calculate Minion Number (1 to 20 total minions)
            int minionNumber = CurrentLevel;
            int totalMinions = 20;

            if (CurrentLevel >= 9 && CurrentLevel <= 14) // Minion Set 2 (Levels 9-14)
            {
                // Minion Set 2 starts after Rest 1 (Level 8). Fights are 8, 9, 10, 11, 12, 13
                // Fights 1-7 are levels 1-7. Level 9 is minion fight 8.
                minionNumber = CurrentLevel - 1;
            }
            else if (CurrentLevel >= 16 && CurrentLevel <= 22) // Minion Set 3 (Levels 16-22)
            {
                // Minion Set 3 starts after Rest 2 (Level 15). Fights are 15 to 21
                // Level 16 is minion fight 14. Wait, the total is 20 fights, not 21.
                // Let's use simple indexing based on fight order:

                if (CurrentLevel <= 7)
                {
                    minionNumber = CurrentLevel; // Fights 1-7
                }
                else if (CurrentLevel <= 14)
                {
                    minionNumber = CurrentLevel - 1; // Fights 8-13 (8 to 13)
                }
                else if (CurrentLevel <= 22)
                {
                    minionNumber = CurrentLevel - 2; // Fights 14-20 (14 to 20)
                }


            }

            // Recalculating the minion number cleanly:
            if (CurrentLevel <= 7) minionNumber = CurrentLevel;           // Levels 1-7 -> Minions 1-7
            else if (CurrentLevel <= 14) minionNumber = CurrentLevel - 1; // Levels 9-14 -> Minions 8-13 (Rest at 8 is skipped)
            else if (CurrentLevel <= 22) minionNumber = CurrentLevel - 2; // Levels 16-22 -> Minions 14-20 (Rests at 8, 15 are skipped)

            // Example: "Goblin 1/20"
            return $"{name} {minionNumber}/{totalMinions}";
        }
    }
    // ------------------------------------------------------------------
    ...
    private void ShowEnemyIntro()
    {
        ...
        // The battle manager should handle displaying enemy name/boss name correctly, 
        // but we pass null for boss here as we're in the enemy intro.
    ...
    private void ShowBossIntro()
    {
        ...
        battleManager.EndBattle();
        // The battle manager should handle displaying enemy name/boss name correctly, 
        // but we pass null for enemy here as we're in the boss intro.
    ...
    private IEnumerator HandleDefeatSequence()
    {
        ...
        {
            // Minion fade
            fadeCoroutine = battleManager.FadeOutCurrentEnemy(3f);
        }
        else if (currentBoss != null)
        {
            // Boss fade
            fadeCoroutine = battleManager.FadeOutCurrentBoss(3f);
        }
    ...
    public void HandleEnemyDefeated()
    {
        ...
        // --- Clean up the destroyed object and clear the reference ---
        if (currentEnemy != null)
        {
            Destroy(currentEnemy.gameObject);
            currentEnemy = null;
        ...
        // Minions are Levels 1-7, 9-14, 16-22
        // Boss is Level 24
        int expGainedThisFight = (CurrentLevel == 24) ? 50 : 10;
    ...
    public void OnPlayerDefeated()
    {
        ...
        // --- Immediately destroy the object on defeat, as no fade happens ---
